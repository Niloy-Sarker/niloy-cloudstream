...
class DflixSeriesProvider : MainAPI() { // all providers must be an instance of MainAPI
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = doc.select(".movie-detail-content-test > h3").text()
        val img = doc.select(".movie-detail-banner > img").attr("src")

        val episodesData = mutableListOf<Episode>()
        var seasonNum = 0
        doc.select("table.table.mb-0 > tbody > tr > th > a").reversed()
            .forEach { season ->
                seasonNum++
    ...
    private suspend fun extractedSeason(
        ...
        seasonDoc.select("div.card.p-4").forEach { episode ->
            val episodeName = episode.select("h4").first()?.ownText()?.trim()
            val episodeImage = episode.parent()?.attr("style")?.let { extractBGImageUrl(it) }
            val episodeDescription = episode.select("div.season_overview > p").text()
            val episodeLink = episode.select("h5 > a").attr("href")
            episodeNum++
            episodesData.add(
