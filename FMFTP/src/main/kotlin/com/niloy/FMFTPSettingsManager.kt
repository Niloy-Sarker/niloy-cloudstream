...
object FMFTPSettingsManager {
    ...
    private const val TAG = "FMFTPSettings"
    ...
    fun isTmdbEnabled(): Boolean {
        ...
        return try {
            // If not in cache, get from SharedPreferences (default to false - disabled until key is set)
            val enabled = getPrefs()?.getBoolean(TMDB_ENABLED, false) ?: false
            cachedTmdbEnabled = enabled // Cache the value
            enabled
        } catch (e: Exception) {
            Log.e(TAG, "Error getting TMDB enabled state: ${e.message}")
            false // Default to disabled if there's an error
        }
    }
